...
let code = require('./worker.js').default.toString()
code = code.substring(code.indexOf('{') + 1, code.lastIndexOf('}'));
console.log(code)
let workerBlob = createBlob([code], {
...
export default class Reader extends Component {
  ...
  check() {
    ...
    vertOffset = (height - resolution) / 2 
    ...
    if (legacyMode || previewIsPlaying) {
      ...
      ctx.drawImage(legacyMode ? img : preview,
        hozOffset, vertOffset, resolution, resolution,
    ...
    return (
      <section className={className} style={style}>
        <section style={containerStyle}>
          ...
          <canvas style={canvasStyle} ref={this.setRefFactory('canvas')} />
        </section>
      </section>
    )
  }
